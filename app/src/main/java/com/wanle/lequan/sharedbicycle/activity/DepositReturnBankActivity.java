...
public class DepositReturnBankActivity extends AppCompatActivity {
    ...
    @BindView(R.id.edit_branch_account)
    EditText mEditBranchAccount;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_deposit_return_bank);
    ...
    private void depositRefund() {
        ...
        String branchAccount = mEditBranchAccount.getText().toString().trim();
        String bankCard = mEditBankCard.getText().toString().trim();
        int checkedId = mRgbRefundReason.getCheckedRadioButtonId();
        ...
        if (TextUtils.isEmpty(realName) || TextUtils.isEmpty(bankAccount)
                ||TextUtils.isEmpty(branchAccount)||TextUtils.isEmpty(bankCard)) {
            ToastUtils.getShortToastByString(this, "请确认所填写的信息是否正确");
        } else {
            Map map = new HashMap();
            String userId = mSpUserInfo.getString("userId", "");
            map.put("userId", userId);
            map.put("rechargeType", "4");
            map.put("realName", realName);
            map.put("bankName", bankAccount);
            map.put("bankBranch",branchAccount);
                        ...
                        if (null != jsonString) {
                            ...
                            if (null != messageBean) {
                                if (messageBean.getResponseCode().equals("1")) {
                                    ...
                                    startActivity(new Intent(DepositReturnBankActivity.this, MyAccountActivity.class));
                                    ToastUtil.show(DepositReturnBankActivity.this,"退款申请已提交");
                                    finish();
                                } else {
                                    ToastUtil.show(DepositReturnBankActivity.this, messageBean.getResponseMsg());
                                }
                            }
                        }
                    } catch (IOException e) {
